...
export const getBarCharts = TryCatch(async (req, res, next) => {
    ...
    if (myCache.has(key))
        ...
        const productCounts = getChartData({ length: 6, today, docArr: products.map(product => product.toObject()) });
        const usersCounts = getChartData({ length: 6, today, docArr: users });
        const ordersCounts = getChartData({ length: 12, today, docArr: orders.map(order => order.toObject()) });
        charts = {
            users: usersCounts,
...
export const getLineCharts = TryCatch(async (req, res, next) => {
    ...
    if (myCache.has(key))
        ...
        const productCounts = getChartData({ length: 12, docArr: products.map(product => product.toObject()), today });
        const usersCounts = getChartData({ length: 12, today, docArr: users });
        const discount = getChartData({
            length: 12,
            today,
            docArr: orders.map(order => order.toObject()),
            property: "discount",
        });
